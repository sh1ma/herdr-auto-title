...
<p align="center">
  <a href="#インストール">インストール</a> · <a href="#タブ名が書き換わるタイミング">書き換え</a> · <a href="#タイトルの言語">タイトルの言語</a> · <a href="#アンインストール">アンインストール</a> · <a href="https://herdr.dev">herdr.dev</a>
</p>
...
タイトルはセッションの最初のプロンプトから生成してタブに書き込む。書き換えるのは、herdr の既定の連番ラベルのままのタブと、この hook が以前に付けたタイトルが残っているタブだけ。自分で付けた名前には触らない。

この hook が名前を付けたタブで新しいセッションを始めたとき（エージェントを終了して同じペインで起動し直す、別のエージェントをそこで開く、など）は、新しいセッションの内容でタイトルを付け直す。どのタブにどのタイトルを付けたかは `~/.claude/herdr-auto-title/tabs/` に覚えておき、自分が付けた名前とユーザーが付けた名前を区別している。
